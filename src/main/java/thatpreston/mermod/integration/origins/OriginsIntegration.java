...
public class OriginsIntegration {
    ...
    public static MermaidTailStyle getTailStyle(Player player) {
        List<Holder<ConfiguredPower<TailStyleConfiguration, TailStylePower>>> powers = IPowerContainer.getPowers(player, TAIL_STYLE.get());
        if(powers != null && powers.size() > 0) {
            TailStyleConfiguration configuration = powers.get(0).get().getConfiguration();
